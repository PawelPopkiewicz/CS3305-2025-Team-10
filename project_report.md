...
> [Github Repository Link](https://github.com/liamcotter/CS3305-2025-Team-10/)

Busig is a cross-platform mobile app which aims to provide a seamless, smooth and simple way to access bus time data, such as the location of buses, stops, schedules and with a focus on predictions. It was developed mainly because me (Radek Zajicek) and Liam Cotter commute every workday and we found the existing solutions not satisfactory.

The main issues we indentified included cluttered, out of date, "laggy" interfaces which would be extra frustrating to use, for example, while holding a cup of coffee and waiting for a bus stop, or when running late to a bus, etc. This frustration guided our front end development and design, instead of for example providing a path finding algorithm to find the route from place A to place B. We already figured Google Maps does a good enough job and most commuters already perfectly know which bus they would like to take and where. So instead we focused on the daily commuters who are already accustomed to the city. This led to a minimal and simple GUI, a focus on performance of the map and a small number of pages.

Another issue was the absurdly inaccurate predictions occasionally given to users. I am sure many people know what we are talking about, but for the lucky among our readers, here is a typical experience: You want to go to work in the morning from a satellite town which should have regular buses every 20 minutes into the city centre, however that is never true and there are many mornings when there is a one-hour gap or more. Secondly when you look at the app the most you can hope for is that it will show you that a bus exists and will eventually arrive, if you are lucky anyway, sometimes they do not exist, sometimes they do exist but they start skipping stops, and mainly, they arrive way later than predicted. We are talking "Arrival in 5 minutes" but you actually wait for 30 minutes. This problem with unpredictable buses is driving the daily commuters away from mass transport, as many studies have shown. Overall, the reasons to look for better predictions and provide them to the public are very relevant and needed.

This led us to mainly focus on our data and the predictions we could make with it. Instead of focusing on routing algorithms, payment of tickets through the app, etc. our predictions are front stage and centre. We needed to dedicate different components just for these predictions and train our own Encoder-Decoder AI model. In the end we have managed to get our predictions working and provide them to the user in a nice simple GUI.

## Glossary

> The terms used that need definitions beyond their colloquial meaning.
...
Our architecture is following the latest trend of microservices, because we realised our program is easy to modularise. This means we would put our code in Docker containers which would then talk to each other through HTTP RESTful APIs implemented in Flask. In the future when our app would be available for the wider public we could use Kubernetes to organise and deploy a cluster of the containers, ensuring scalability, elasticity and robustness. Especially if we would choose a cloud solution such a Google Kubernetes Engine. For now however, all of the containers are orchestrated using the Docker Compose functionality. Implemented in the docker-compose.yml file. This takes care of setting up a shared network among the containers, building them, adding variables to their environments, persisting data using Docker Volumes which maps a part of the local file space to the space in the container and more. Instead of cloud, our app currently runs on a home Ubuntu server which is enough for development needs.
...
- Gateway
  ...
  - Serves as a proxy to decouple the front facing responsibility from the Bus model container
- Training Data Collection
  - Receives the JSON real-time data from the Bus model after it is fetched
  - Processes the data and stores it in Mongodb container
  - Provides APIs to return the contents of the Mongodb, mainly to the Inference container
- Inference
  - Handles preprocessing of the training data
...
One of the main issues we had to deal with was inconsistent data. The inconsistency comes from the static data mainly. Of course real-time data can report a bus which is off-route, not scheduled, etc., but that is to be expected and can be filtered out. However static data is provided as a ZIP file in CSV formatted text files online. This is okay as we can simply download the file, unzip it, process the CSV data, turn it into a relational tables and use it. However the main issue is that id fields which are used as primary keys **CHANGE**. And they change frequently and without notification. This means a route id for the route "220" might change overnight with no reason to, the trip id changes as well, but at a different rate, etc.

The way to combat this was to build the PostgreSQL periodically, let's say every day, to prevent out-of-date ids. This means the creation of the tables needed to be optimised for speed, because there are millions of rows to process, filter out, index, etc. This was eventually achieved by constructing optimised filter queries, using pandas and constructing indexes for fast lookups.
...
The Bus model container runs a Flask server which primarily manages the transfer of data around the back-end and fulfills requests from the front-end. The data transfer aspect is more deeply discussed in the [Data Flow](#flow-events-triggering-changes-in-the-architecture) section.
...
| stops.txt          | Specifies details about stops                               | 10500              |
| stop_times.txt     | Specifies the time each trip arrives at each stop           | 6.3M               |
...
The last aspect I want to talk about is the normalisation of values. This essentially means that the values should be in a similar range. For this reason I subtract the current delay from all of the times fed into the model, meaning the relative error is always 0 at the start, this helps with cases like an hour delay or more, which might throw off the model, even though the actual trip has normal times between stops. Of course this delay is then added back after inference.
...
One of our main goals on the front-end was to ensure the user has a smooth and intuitive experience when navigating the lightweight app. Hence, we decided on the screen layout that centers around the map - the centerpiece of the application, that we ensured is readable and works smoothly - unlike the maps of many of our alternatives. Wherever possible, we tried to use familiar user flows, to improve the intuitive feel even further.

We ensured the first-time user can get to the key information they want within 2 clicks, to make the app accessible and avoid any potential confusion. A typical flow is as follows: start on the empty home page -> navigate to the map (see the locations of buses and stops) -> click a bus/stop (see the selected schedule)

Additionally, to lower mobile data usage, we optimised the API usage to only need 3 calls for setup and as low as 1 API call per minute when keeping the app open, while still showing accurate data. When dealing with the schedule data, we only use API calls for the strictly necessary information, saving resources and making the app more responsive. We also utilised `useFocusEffect` to deal with the schedule screens calling APIs when off-screen.

Despite initial ideas of adding an account based system, we decided against it, as we weighed the user benefit of being able to share their data between their devices against the risk of frustrating the first-time user that just wants to check the timetable.
...
We wanted to ensure that as many parts of the app were reusable - hence modular components that are used throughout multiple screens. Additionally, the programmatic and centralised implementation of the app theme allows for quick iterations of design, as the changes are reflected throughout the entire app. By utilising Redux and global state management, we ensure our components can access the necessary data regardless of their position in the app, avoiding excessive parameter passing, which can cause major issues when refactoring in React Native.

The above-mentioned techniques let our development stay agile even as the project increases in scope.
...
When users tap on a bus stop on the map, detailed information is displayed, including upcoming bus arrival times, route numbers, and destinations. This feature is designed to give quick answers to common commuter questions like “When is my bus arriving?”

##### Live Bus Tracking

Users already on a bus can select the bus directly from the map to view live details such as the current location and estimated time to upcoming stops. This is especially useful for planning when to get off the bus.
...
To help accommodate frequent users, the app allows you to save your favorite bus routes or stops. By tapping the star icon on any stop or bus, users can add it to their Home Screen for faster access in the future. The data is persistent between sessions due to the usage of AsyncStorage, that saves the data on the local device and repopulates it on startup.

##### Map Filtering

Users can filter the displayed data on the map to show only the selected bus routes or stops. This reduces clutter and allows for a more personalised and focused map view based on the user’s needs. Using this feature also further improves performance, as there are fewer objects to perform calculations on.
...
| Client update    | Client queries the gateway for updated bus data, it then fetches this info from the bus model                                                           | ~1 min     |
| Client request   | Client requests more detailed info, queries the gateway which then queries the bus model                                                            | Unscripted |
...
- Providing extra context for front-end users
  - We can tell the front-end when a bus goes off route, is cancelled, or whether the predicted times are model-predicted or estimated based on delay, etc.
- Web version
...
- Reducing response time
  - There are many potential changes in the back-end architecture which could reduce the server boot time, the response time for the clients or inference response time such as using Redis for caching.
- Asynchronous inference
  - We would like to create a data processing pipeline which can, amongst other uses, infer predictions in an asynchronous manner to both speed up and reduce the responsibilites of the bus model container.
- Test coverage
  - Now that we have a minimum viable product, it would be ideal to have test coverage as a part of a CI/CD pipeline.
...
- Expanding to other data sources
  - Theoretically, any transport authority which provides data adhering to the GTFS is compatible with our back-end, which includes foreign public transport and even trains and the Luas in Ireland!
...
Installed Ubuntu server on my old laptop, set up the networking on that laptop to provide a static IP address and also implemented basic security with firewall and ssh. I then ran and managed the server for the duration of the training data collection, sometimes debugging and troubleshooting it.
...
I have also created the architecture diagram and played a big role in designing the architecture because I was in charge of the containerisation. And as everyone else I have worked on the presentation, helped with the design of the one page report and I wrote most of the project report, except for the containers and contributions which I have not worked on personally.
...
I wrote most of the content of the one page report where I was knowledgeable of the details. I extensively proof-read and made minor corrections to this project report, so you can blame me for any mistakes that have remained unnoticed.

###  Pawel

Analysed pain points of users of similar products to ensure positive and accessible UX.
...
Took full ownership for architecture related complex parts of the front-end - ranging from advanced data integration to performance optimisation.
...
Standardised TypeScript types to maintain consistency with evolving backend documentation and reduce runtime errors.

Optimised API calls and integrated AsyncStorage persistent storage, resulting in a convenient user experience and a smoother user interface.
...
To improve performance, I optimised the map view by caching data such as buses and stops locally and minimising unnecessary re-renders. This significantly improved responsiveness and overall app quality.

I developed and standardised the main UI components, including interactive map markers, bus information pages, and stop detail views. These components followed a consistent design pattern to enhance maintainability and usability.
